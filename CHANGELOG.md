...
- Removed the **LLM Generated Pattern** mode from Regex Extractor. It called `POST /generate_pattern`, which is not and has never been a Crawl4AI Docker REST endpoint — `generate_pattern` only exists as a Python SDK classmethod (`RegexExtractionStrategy.generate_pattern`), which this REST-API-only package cannot reach. Every use of this mode has always failed with a 404. Built-in, custom, and both presets are unaffected. A saved workflow referencing the removed mode now gets a clear error explaining the removal instead of silently falling into the custom-patterns branch.
- `getJobStatus` never surfaced a failed job's error text at all — Crawl4AI's `/crawl/job/{task_id}` and `/llm/job/{task_id}` return the failure reason in an `error` field on the response, which the `JobStatusResponse` type didn't declare and the operation never read. A failed async job produced only `taskId`/`status`/`checkedAt` with no indication of what went wrong. Now surfaced as `error` in the output, with the same untrusted-config-rejection explanation applied as the synchronous path (see below).
...
- `parseApiError` now recognises Crawl4AI's untrusted-request-rejection message (`"... is not permitted ... from an untrusted request"` / `"... may not be constructed from an untrusted request"`) and returns an actionable explanation instead of a generic "bad request" message. Extracted as `explainUntrustedConfigRejection()` so both the synchronous 400 path and the async `getJobStatus` failed-job path share one implementation.

### Compatibility — Crawl4AI v0.9.0+ Docker API hardening
Crawl4AI v0.9.0 made the Docker API server secure-by-default (verified directly against `unclecode/crawl4ai`'s `deploy/docker/` and `crawl4ai/async_configs.py` source — the exact forbidden-field lists, allowlists, and error message formats, not just the migration doc's prose summary). Two changes affect this package, both enforced server-side with no client-side workaround:
- Authentication is on by default: `entrypoint.sh` binds the server to loopback only when no `CRAWL4AI_API_TOKEN` is set, which makes it unreachable from n8n (not just restricted) for the default cross-container Docker setup this package documents. This package's existing Token Authentication credential already sends the required `Authorization: Bearer` header — no code change needed, only server-side env var + credential configuration.
- Every `/crawl` (and `/crawl/job`) request body is now validated as untrusted: Crawl4AI rejects `js_code`, `cookies`, `headers`, `proxy`/`proxy_config`, `extra_args`, `init_scripts`, `user_data_dir`, `cdp_url`, `chrome_channel`, `storage_state`, `session_id`, `deep_crawl_strategy`, `magic`, `simulate_user`, `override_navigator`, and any embedded `LLMExtractionStrategy`/`LLMContentFilter`/`LLMTableExtraction` with HTTP 400 (confirmed field-by-field against `UNTRUSTED_FORBIDDEN_FIELDS`/`UNTRUSTED_ALLOWED_TYPES` in `crawl4ai/async_configs.py`). This breaks **Ask Question**, **Extract Data**, **LLM Extractor** (all embed `LLMExtractionStrategy`), **Process Raw HTML** (always sends `base_url`, unconditionally broken, not just with certain options), and any operation using Magic Mode, Simulate User, Override Navigator, cookies, custom headers, a proxy, a session ID, storage state, a browser channel, deep-crawl scopes, or an LLM Content Filter/LLM Table Extraction, when run against Crawl4AI v0.9.0+. Enable Stealth Mode is unaffected (it's on Crawl4AI's allowlist, not its forbidden list) — an earlier draft of this changelog incorrectly listed it as broken. A few options (`ignore_https_errors`, `use_managed_browser`) are silently dropped rather than rejected, so their absence from an error doesn't mean they took effect. This is a deliberate upstream security boundary (it blocks the same request shape regardless of caller), not a defect in this package. Full functionality requires Crawl4AI v0.8.9 or earlier; see README's "Crawl4AI Version Compatibility" section for the full breakdown of what still works on v0.9.0+.
